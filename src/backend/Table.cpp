...
Table::Table(string name, Pager &pgr, size_t tablePageId) : m_pager(pgr), m_tablePageID
    ...
    if (T_PAGE.getBtreePageID() == cts::SIZE_T_OUT_OF_BOUNDS) {
        ...
        m_pager.createNewPage<BtreeNodePage<vector<variants>>>(minDeg, cts::SIZE_T_OUT_OF_BOUNDS,
                                                            true, true);
    }

    m_btree = std::make_unique<Btree<vector<variants>>>(T_PAGE.getBtreePageID(), pgr);
}
